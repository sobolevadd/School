...
Prvek* Seznam::najdiNejmensi(Prvek* od, bool (*mensiNez)(void*p1, void* p2))
{
	Prvek * pom = od;
	T hod = pom->getData();
	while(od != konec)
...
void Seznam::setrid(bool (*mensiNez)(void*p1, void* p2))
{
	Prvek* od = hlava;
	while(od != konec)
	{
		Prvek * nejmensi = najdiNejmensi(od, mensiNez);
		if(nejmensi != od)
			prohod(nejmensi, od);
		od = od->getDalsi();
	}
