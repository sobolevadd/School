...
def test_scenarios_from(portal:IPkgInitor, LID:Level=99) -> str:
    ...
    LEVEL = LID
    _portal = portal
    # dbg.prIN(1, f'{LEVEL = },   Author = {_portal.AUTHOR_NATIVE_NAME}')
    n2s = portal.NAME_2_SCENARIO
    if not isinstance(n2s, dict):
        n2s = n2s()
    scen_view = n2s.values()
    num       = len(scen_view)
...
def _initialize() -> None:
    ...
    prVb(Verbosity.DETAILS, f'{N_HASH_N}'
         f'Scenario: {_scenario_in_test.name}{N_DOUBLE}')
...
def _verify_needs():
    ...
    if not expects:
        _ERR(f'Krok nemá zadány očekávané příznaky',
             steps=1)
    for key in expects:
        if (key == 'conditions') or (key == 'tests'):
