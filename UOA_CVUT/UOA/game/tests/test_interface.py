...
def test(portal:IPkgInitor, level:Level, visitorClass:type=Visitor) -> bool:
    ...
    """
    # dbg.prIN(1, f'{portal.AUTHOR_NATIVE_NAME} - {level=} - '
    #             f'visitor={visitorClass.__name__}')
    ...
    try:
        ...
        if level >= Level.HAPPY:
            ...
            #     epilog     += f'{HASH_N}{invitation}{N_DOUBLE}'
        ...
        if level >= Level.START:
            ...
            from .test_game import test_game_from
            test_game_from(portal, level, visitorClass)
            # dbg.prIN(1, 2*(66*'P' + '\n'))
