...
# @dbg.prSEda()
def magic(arguments:tuple[str]) -> str:
    """
    Ověří, že se v aktuálním prostoru nachází objekt, s nímž je možno
    komunikovat, a poku ještě nebyl buzen, tak jej vzbudí.
    """
    if len(arguments) < 2:
        return 'Příkaz MAGIE neznám.'                   # Není zadán argument
    item = '_'+arguments[1]
    if (conditions['MAGIC'] and arguments[1] == 'kouzlo'):
        world.BAG.MojeVelikost += 1
        conditions['MAGIC'] = False
        world.BAG.add_item(world.Item(item))
        return 'Dosažení cíle vám usnadní právě získaný objekt KOUZLO'
    if (not conditions['MAGIC'] and conditions['NOEND']
     and arguments[1] == 'konec'):
        conditions['NOEND'] = False
        world.BAG.remove_item('kouzlo')
        world.BAG.MojeVelikost -= 1
        world._current_place = world.place('Kuchyn')
        conditions['brambory.inkitchen'] = True
        #world.BAG.remove_item('Seno')
        A = 'Vše je připraveno pro to, abyste'
        B = ' zadáním příkazu POPŘEJ dosáhli KONEC.'
        return A+B
    else:
        return 'Příkaz MAGIE neznám.' 
...
def wish(arguments:tuple[str]) -> str:
    ...
    if not (item := payable_present(arguments)):
        return NOT_SUCCESa                  # Není komu přát
    if conditions[item.name.lower() + '.inkitchen']:
        global is_active
        is_active = False
        return SUCCESS1 + item.name + SUCCESS2   # Objektu jsme popřáli
    else:      # Objekt ještě nebyl pozdraven
